...
class AssessmentController {
    async createAssessment(req: Request, res: Response) {
        try {
            ...
            if (isNaN(startTime.getTime()) || isNaN(endTime.getTime())) throw new Error("Invalid startTime or endTime");
            ...
            return res.status(200)
                .json(apiResponse(200, "Assessment Created Successfully.", createdAssessment))
        } catch (error: any) {
            console.log(error);
        ...
        try {
            ...
            const updatedAssessment = await prismaClient.assessment.update({
                ...
                data: {
                    ...
                    individualSectionTimeLimit: data.individualSectionTimeLimit ?? assessment.individualSectionTimeLimit,
                    status: data.status ?? assessment.status,
                    batchId: data.batchId ?? assessment.batchId,
                }
            });
            if (!updatedAssessment) throw new Error("Error Updating Assessment");
            return res.status(200)
                .json(apiResponse(200, "Assessment Updated Successfully.", updatedAssessment));
        } catch (error: any) {
            console.log(error);
            return res.status(200).json(apiResponse(200, error.message, null));
        }
    ...
    async deleteAssessment(req: Request, res: Response) {
        try {
            ...
            return res.status(200)
                .json(apiResponse(200, "Assessment Deleted Successfully.", deletedAssessment));
        } catch (error: any) {
            console.log(error);
            return res.status(200).json(apiResponse(200, error.message, null));
        }
